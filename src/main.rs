...
fn read_label_file(path: &std::path::Path) -> Vec<u8> {
...
fn test_data() -> DataSet{
    DataSet{label: read_test_label(), image: read_test_image() }
}

fn train_data() -> DataSet{
    DataSet{label: read_train_label(), image: read_train_image() }
}
...
fn read_image_file(path: &std::path::Path) -> Vec<[u8; 28*28]> {
    let mut f = BufReader::new(fs::File::open(path).unwrap());
    let mut n: i32 = 0;
    let buf = unsafe{slice::from_raw_parts_mut(&mut n as *mut i32 as *mut u8, 4) };
    f.read_exact(buf).unwrap();
